...
#
# ⚠ 不可绕过的问题：api_get 返回的是解析后的 JSON，**不暴露响应头**。
...
def probe_quota():
    ...
    req = urllib.request.Request(
        API + "/health", headers={"User-Agent": "Mozilla/5.0"})
    try:
...
def rate_limited_get(path, params=None, tries=3):
    ...
    ★ 实测 OpenDota 会偶发返回 500/502/503，且可能连续两三次。
      api_get 内部对 5xx 只做固定 2s 间隔重试，连续抖动时仍会失败。
      此处在其外层再加一层**指数退避**（2s / 5s），显著降低瞬时抖动导致的失败率。
      注意：只对 5xx 重试；404 等语义错误立即抛出，不做无谓等待。

    ★ 配额观测不走 api_get（它吞掉响应头），而是本层直接发一次请求。
...
def _get_with_quota(path, params=None):
    """
    与 api_get 同语义的单次 GET，但会读取响应头写入配额。
...
def run_mate_job(job_id, req):
    ...
    try:
        ...
        aid_a, desc_a = to_account_id(req.get("player_a", ""))
        aid_b, desc_b = to_account_id(req.get("player_b", ""))
        if aid_a == aid_b:
...
def run_scan_job(job_id, req):
    ...
    try:
        ...
        set_stage(job_id, "validate", "校验玩家信息…")
        aid_base, desc_base = to_account_id(req.get("player_base", ""))
        aid_tgt, desc_tgt = to_account_id(req.get("player_target", ""))
        if aid_base == aid_tgt:
            raise ValueError("两个 ID 是同一个账号。")
